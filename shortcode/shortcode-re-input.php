<?php
    ...
    $str = '';
    $str .= '<p class="cfbf-message">'.nl2br($cfbfs_o['re-top-message']).'</p>';
    $str .= '<div id="cfbf"><form method="POST"><table class="cfbf-wrapper"><tbody>';
    $str .= '<tr>';
    $str .= '<td class="cfbf-title">'.__('Message').'<span class="req">'.__('Required', 'contact-form-by-fulfills').'</span></td>';
    $str .= '<td class="cfbf-inputs"><textarea name="cfbf_reinput[message]" cols="50" rows="10" required></textarea></td>';
    $str .= '</tr>';
    $str .= '</tbody></table>'.wp_nonce_field('cfbf_form', 'cfbf_reinput[nonce]').'<input type="submit" value="'.__('SEND').'">';
    ...
    $str .= '<input type="hidden" name="cfbf_reinput[cfbfid]" value="'.$_GET['cfbfid'].'" />';
    $str .= '<input type="hidden" name="cfbf_reinput[auth1]" value="'.$_GET['auth1'].'" />';
    $str .= '<input type="hidden" name="cfbf_reinput[auth2]" value="'.$_GET['auth2'].'" />';
    $str .= '</form></div>';
    return $str;